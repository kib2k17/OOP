...
def guests():
    ...
    if request.method == 'POST':
        ...
        if action == 'add':
            ...
            if selected_room and selected_room != "N/A":
                for room in rooms_db:
                    if str(room['id']) == str(selected_room):
            ...
            for guest in guests_db:
                if str(guest['id']) == str(guest_id):
                    assigned_room = guest.get('room')
                    if assigned_room and assigned_room != "N/A":
                        for room in rooms_db:
                            if str(room['id']) == str(assigned_room):
                                room['status'] = 'Available'
                                break
                    archive_deleted_item('guest', guest)
                    break
            guests_db = [g for g in guests_db if str(g['id']) != str(guest_id)]
    ...
    active_assigned_rooms = [
        str(g['room']) for g in guests_db 
        if g.get('room') and g['room'] != 'N/A' and g.get('status') in ['Checked In', 'Reserved']
    ]
    
    data_changed = False
    for room in rooms_db:
        if str(room['id']) in active_assigned_rooms:
    ...
    query = request.args.get('search', '').lower()
    filtered_guests = [
        g for g in guests_db 
        if query in str(g['id']).lower() or query in g['name'].lower()
    ] if query else guests_db

    available_rooms = [r for r in rooms_db if r.get('status') == 'Available' and not r.get('is_deleted', False)]
